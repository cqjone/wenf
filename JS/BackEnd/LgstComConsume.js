Ext.onReady(function () {
	...
	Ext.Ajax.request({
	    url: '../../Data/iEmployee.json',
	    // params: {
	    // 	id: 1
	    // }
	    success: function(data) {
	    	var empData = Ext.decode(data.responseText).iEmployee;
	    	var empInfo = [];
	    	for (var i = 0, len = empData.length; i < len; i++) {
	    		var temp = {};
	    		temp.emp = empData[i].Code + ' ' + empData[i].Title;
	    		empInfo.push(temp);
	    	}
	    	storeEmp.loadData(empInfo);
	    }   
	});
		...
		items: [
			...
			},
			{
				...
				columns: [
					...
					{header: '数量', dataIndex: 'amount', sortable: true, width: 90, css: 'text-align: right;', renderer: formatDigits, editor: editor},
					{header: '操作', width: 60,
				        // 添加删除列
	...
	// 提交按钮
	function submit() {
		if (Ext.getCmp('emp').getValue() == '') {
			Ext.getCmp('emp').focus();
			return;
		} else if (Ext.getCmp('use').getValue() == '') {
			Ext.getCmp('use').focus();
			return;
		}
		...
		// 数据
		var emp = Ext.getCmp('emp').getValue();
