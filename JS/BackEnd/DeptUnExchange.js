...
var cm = new Ext.grid.ColumnModel({
    ...
    //sm,    
    {
        ...
        align: "center",
        renderer: function () {
            return "<a href='#' onclick='doSendOut();'>兑 换</a>";
        }
    }, {
        header: "",
        align: "center",
        hidden:true,
        renderer: function () {
            return "<a href='#' onclick='notifyLog();'>通知物流部发货</a>";
        }
    }
    ]
});
...
function notifyLog() {
    ...
    Ext.Msg.confirm("提示框", "是否确定将该预约订单的取货方式改成'快递'？", function (btn) {
        if (btn == "yes") {
            Ext.Ajax.request({
                url: '../Apis/PointExchange.aspx?actionName=updateGetType&sid=' + Sys.sid,
                params: "id=" + r[0].get("ID") +
                "&getType=1",
...
function doSendOut() {
    ...
    sendOutForm.find("name", "Quantity")[0].setValue(rows[0].get("Quantity"));
    sendOutForm.find("name", "OnlineName")[0].setValue(rows[0].get("OnlineName"));
    sendOutForm.find("name", "TotalCount")[0].setValue(rows[0].get("TotalCount"));
    sendOutForm.find("name", "ReservationDate")[0].setValue(ConvertJSONDateToJSDateObject(rows[0].get("ReservationDate")));
    //alert(rows[0].get("ReservationDate"));
}
    ...
    buttons: [{
        ...
        handler: function () {
            if (pwdform.getForm().isValid()) {
                ...
                sendOutForm.find("name", "Password")[0].setValue(pwd);
                UpdatebPointExchange();
            }
        }
    }, {
...
var pd_store = new Ext.data.Store({
    autoDestroy: true,
    //autoLoad: true,
    url: '../Apis/PointExchange.aspx?actionName=getDeptUnExchange&sid=' + Sys.sid,
...
pd_store.on("loadexception", function (mis) {
    Ext.MessageBox.alert("提醒", "未查询到数据！");
    pd_store.removeAll();
});
