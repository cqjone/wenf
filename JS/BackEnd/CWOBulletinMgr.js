Ext.onReady(function () {
    var DeptID = 0;
    var Month = 0;
    var cw_store = new Ext.data.Store({
        autoDestroy: true,
    ...
    var cwMonthStore = new Ext.data.ArrayStore({
        fields: ['ID', 'Title'],
        data: [["1", "一月"],
            ["2", "二月"],
            ["3", "三月"],
    ...
    var Form_CWOBullentinGroup = new Ext.form.FormPanel({
                        ...
                        items: [
                            {
                                ...
                                listeners: {
                                    'click': function () {
                                        DeptID = Form_CWOBullentinGroup.find('name', 'DeptID')[0].value;
                                        Month = Form_CWOBullentinGroup.find('name', 'Month')[0].value;
                                        store_CWOBullentinGroup.load({
                                            params: { DeptID: DeptID, Month: Month,ReportType:0 }
                                        });
                                    }
                                }
                            }
    ...
    var store_CWOBullentinGroup = new Ext.data.JsonStore({
    ...
    var grid_CWOBullentinGroup = new Ext.grid.GridPanel({
        store: store_CWOBullentinGroup,
        stripeRows: true,
        id: 'CWOBullentinGrid',
        cm: new Ext.grid.ColumnModel({
            ...
            columns: [
                ...
                {
                    header: '操作',
                    renderer: showbutton_CWOBullentinGroup
                }
            ]
        })
    });
    ...
    function showbutton_CWOBullentinGroup(value, metadata, record, rowIndex, columnIndex, store) {
        window.DownLoadCWOBullentin = function (rowIndex) {
            var record = Ext.getCmp("CWOBullentinGrid").getStore().getAt(rowIndex);
            Ext.Ajax.request({
        ...
        var resultStr = "<a href='#' onclick='DownLoadCWOBullentin(" + rowIndex + ")'>下载</a>";
        return resultStr;
    }
    ...
    var pd_main_panel = new Ext.Panel({
        ...
        items: [{
            frame: true,
            border: false,
            items: [Form_CWOBullentinGroup]
        }, {
            layout: "fit",
            border: false,
            anchor: '-1 -100',
            items: [grid_CWOBullentinGroup]
        }]
    });
